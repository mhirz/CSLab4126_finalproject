...
class RegistrationForm(FlaskForm):
    ...
    firstname = StringField("", validators=[DataRequired(), Length(min=2, max =20), Regexp("^\w+$", message="Dein Name darf nur Buchstaben enthalten")], render_kw={"placeholder": "Vorname"})
    lastname = StringField("", validators=[DataRequired(), Length(min=2, max=20), Regexp("^\w+$", message="Dein Name darf nurt Buchstabebn enthalten")], render_kw={"placeholder": "Nachname"})
...
class LoginForm(FlaskForm):
    email = StringField("", validators=[DataRequired()], render_kw={"placeholder": "Email Adresse"})
...
class ForgotPw(FlaskForm):
    firstname = StringField("", validators=[DataRequired(), Length(min=2, max =20)], render_kw={"placeholder": "Vorname"})
    lastname = StringField("", validators=[DataRequired(), Length(min=2, max =20)], render_kw={"placeholder": "Nachname"})
    email = StringField("", validators=[DataRequired(), Email(message="Gültige Email-adresse erforderlich")], render_kw={"placeholder": "Email Adresse"})
